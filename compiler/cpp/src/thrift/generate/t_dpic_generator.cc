 ...
 *
 */
class t_dpic_generator : public t_oop_generator {
public:
  t_dpic_generator(t_program* program,
  ...
  void generate_dpic_extern(ostream& out,
                            t_struct* tstruct);
...
private:
  ...
  // The DpicProcessorGenerator is used to generate parts of the code,
  ...
  // themselves.
  friend class DpicProcessorGenerator;
};
 ...
 * streams.
 */
void t_dpic_generator::init_generator() {
  ...
  f_types_impl_ << "#include <stdio.h>\n";
  f_types_impl_ << "#include <stdlib.h>\n";
  f_types_impl_ << "#include <thrift/protocol/TBinaryProtocol.h>\n";
  f_types_impl_ << "#include <thrift/protocol/TJSONProtocol.h>\n";
  f_types_impl_ << "#include <thrift/transport/TBufferTransports.h>\n\n";

  f_types_impl_ << "#include <iostream>\n";
  f_types_impl_ << "using namespace apache::thrift;\n";
  f_types_impl_ << "using namespace apache::thrift::protocol;\n";
  f_types_impl_ << "using namespace apache::thrift::transport;\n";
 ...
 */
void t_dpic_generator::close_generator() {
  // Close namespace
  f_types_ << ns_close_ << '\n' << '\n';
 ...
 */
void t_dpic_generator::generate_typedef(t_typedef* ttypedef) {
  generate_java_doc(f_types_, ttypedef);
  f_types_ << indent() << "typedef " << type_name(ttypedef->get_type(), true) << " "
           << ttypedef->get_symbolic() << ";" << '\n' << '\n';
}

void t_dpic_generator::generate_enum_constant_list(std::ostream& f,
                                                  const vector<t_enum_value*>& constants,
                                                  const char* prefix,
                                                  const char* suffix,
                                                  bool include_values) {
 ...
 */
void t_dpic_generator::generate_enum(t_enum* tenum) {
...
void t_dpic_generator::generate_enum_ostream_operator_decl(std::ostream& out, t_enum* tenum) {
...
void t_dpic_generator::generate_enum_ostream_operator(std::ostream& out, t_enum* tenum) {
...
void t_dpic_generator::generate_enum_to_string_helper_function_decl(std::ostream& out, t_enum* tenum) {
...
void t_dpic_generator::generate_enum_to_string_helper_function(std::ostream& out, t_enum* tenum) {
  if (!has_custom_ostream(tenum)) {
    out << "std::string to_string(const ";
    if (gen_pure_enums_) {
      out << tenum->get_name();
 ...
 */
void t_dpic_generator::generate_consts(std::vector<t_const*> consts) {
 ...
 */
void t_dpic_generator::print_const_value(ostream& out,
                                        string name,
                                        t_type* type,
                                        t_const_value* value) {
 ...
 *
 */
string t_dpic_generator::render_const_value(ostream& out,
                                           string name,
                                           t_type* type,
                                           t_const_value* value) {
  (void)name;
...
void t_dpic_generator::generate_forward_declaration(t_struct* tstruct) {
 ...
 */
void t_dpic_generator::generate_cpp_struct(t_struct* tstruct, bool is_exception) {
  ...
  generate_dpic_extern(f_types_impl_, tstruct);


  has_members_ = true;
}

void t_dpic_generator::generate_equality_operator(std::ostream& out, t_struct* tstruct) {
  // Get members
...
bool t_dpic_generator::has_field_with_default_value(t_struct* tstruct)
{
  vector<t_field*>::const_iterator m_iter;
  const vector<t_field*>& members = tstruct->get_members();
...
void t_dpic_generator::generate_default_constructor(ostream& out,
                                                   t_struct* tstruct,
                                                   bool is_exception) {
  // Get members
  vector<t_field*>::const_iterator m_iter;
...
void t_dpic_generator::generate_copy_constructor(ostream& out,
                                                t_struct* tstruct,
                                                bool is_exception) {
  generate_constructor_helper(out, tstruct, is_exception, /*is_move=*/false);
}

void t_dpic_generator::generate_move_constructor(ostream& out,
                                                t_struct* tstruct,
                                                bool is_exception) {
...
void t_dpic_generator::generate_constructor_helper(ostream& out,
                                                  t_struct* tstruct,
                                                  bool is_exception,
                                                  bool is_move) {
...
void t_dpic_generator::generate_assignment_operator(ostream& out, t_struct* tstruct) {
  generate_assignment_helper(out, tstruct, /*is_move=*/false);
}

void t_dpic_generator::generate_move_assignment_operator(ostream& out, t_struct* tstruct) {
  generate_assignment_helper(out, tstruct, /*is_move=*/true);
}

void t_dpic_generator::generate_assignment_helper(ostream& out, t_struct* tstruct, bool is_move) {
  std::string tmp_name = tmp("other");
...
void t_dpic_generator::generate_dpic_extern(ostream& out,
                                            t_struct* tstruct) {
  ...
  indent_up();
  indent(out) << "void display_" << tstruct->get_name() << "(" << tstruct->get_name() << "* obj) {\n";
  indent_up();
  indent(out) << "obj->printTo(std::cout);\n"; 
  indent(out) << "std::cout << \"\\n\";\n";
  indent_down();
  indent(out) << "}\n\n\n";
  indent_down();
  ...
  indent_up();
  indent(out) << tstruct->get_name() << "* deserialize_" << tstruct->get_name() << "_from_file(const char *filename) {\n" ;
  indent_up();
  indent(out) << "FILE *file = fopen(filename, \"rb\");\n";
  indent(out) << "if (!file) {\n";
  indent_up();
  indent(out) << "perror(\"can not open thrift file.\\n\");\n";
  //indent(out) << "return;\n";
  indent_down();
  indent(out) << "}\n";
  indent(out) << "else {\n";
  indent_up();
  indent(out) << "printf(\"[THRIFT-DPIC]Start to read file: \%\s\\n\", filename);\n";
  indent_down();
  indent(out) << "}\n\n";

  indent(out) << "fseek(file, 0, SEEK_END);\n";
  indent(out) << "long length = ftell(file);\n";
  indent(out) << "fseek(file, 0, SEEK_SET);\n\n";
  indent(out) << "uint8_t *data = (uint8_t *)malloc(length);\n";
  indent(out) << "fread(data, 1, length, file);\n";
  indent(out) << "fclose(file);\n\n";
  
  indent(out) << "std::shared_ptr<TMemoryBuffer> transport = std::make_shared<TMemoryBuffer>((uint8_t *)data, length);\n";
  indent(out) << "TJSONProtocol protocol(transport);\n\n";

  //indent(out) << tstruct->get_name() << " object;\n";
  indent(out) << tstruct->get_name() << "* object = new " << tstruct->get_name() << "();\n";
  indent(out) << "object->read(&protocol);\n";

  indent(out) << "free(data);\n";
  indent(out) << "return object;\n";
  indent_down();
  ...
  for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
    string struct_name = tstruct->get_name();
    ...
    if (type->is_base_type()) {
        ...
        switch (tbase) {
        case t_base_type::TYPE_STRING:
          indent(out) << "void read_" << struct_name << "_" << field_name << "(" << struct_name <<"* obj" << ", " 
                      <<  "const char** " << field_name << ") {\n";
          indent_up();
          indent(out) << "std::cout << obj->" << field_name << " << std::endl;\n";
          indent(out) << "*" <<field_name << " = obj->" << field_name << ".c_str();\n";
          indent_down();
          indent(out) << "}\n\n";
          break;
        case t_base_type::TYPE_I32:
          ...
          //indent_up();
          //indent(out) << "std::cout << obj->" << field_name << " << std::endl;\n";
          //indent(out) << "*" << field_name << " = obj->" << field_name << ";\n";
          //indent_down();
          //indent(out) << "}\n\n";
          //break;
        ...
        default:
          indent(out) << "void read_" << struct_name << "_" << field_name << "(" << struct_name <<"* obj" << ", " 
                      <<  type_name((*m_iter)->get_type()) << "* " << field_name << ") {\n";
          indent_up();
          indent(out) << "using ::apache::thrift::to_string;\n";
          
          indent(out) << "std::cout << to_string(obj->" << field_name << ") << std::endl;\n";
          indent(out) << "*" << field_name << " = obj->" << field_name << ";\n";
          indent_down();
          indent(out) << "}\n\n";
          break;
 ...
 */
void t_dpic_generator::generate_struct_declaration(ostream& out,
                                                  t_struct* tstruct,
                                                  bool is_exception,
                                                  bool pointers,
                                                  bool read,
...
void t_dpic_generator::generate_struct_definition(ostream& out,
 ...
 * @param tstruct The struct
 */
void t_dpic_generator::generate_struct_reader(ostream& out, t_struct* tstruct, bool pointers) {
  if (gen_templates_) {
    out << indent() << "template <class Protocol_>" << '\n' << indent() << "uint32_t "
 ...
 * @param tstruct The struct
 */
void t_dpic_generator::generate_struct_writer(ostream& out, t_struct* tstruct, bool pointers) {
 ...
 */
void t_dpic_generator::generate_struct_result_writer(ostream& out,
                                                    t_struct* tstruct,
                                                    bool pointers) {
  string name = tstruct->get_name();
  const vector<t_field*>& fields = tstruct->get_sorted_members();
 ...
 * @param tstruct The struct
 */
void t_dpic_generator::generate_struct_swap(ostream& out, t_struct* tstruct) {
  if (tstruct->get_name() == "a" || tstruct->get_name() == "b") {
    out << indent() << "void swap(" << tstruct->get_name() << " &a1, " << tstruct->get_name()
        << " &a2) {" << '\n';
  } else {
...
void t_dpic_generator::generate_struct_ostream_operator_decl(std::ostream& out, t_struct* tstruct) {
  out << "std::ostream& operator<<(std::ostream& out, const "
      << tstruct->get_name()
      << "& obj);" << '\n';
  out << '\n';
...
void t_dpic_generator::generate_struct_ostream_operator(std::ostream& out, t_struct* tstruct) {
  if (!has_custom_ostream(tstruct)) {
...
void t_dpic_generator::generate_struct_print_method_decl(std::ostream& out, t_struct* tstruct) {
  out << "void ";
  if (tstruct) {
    out << tstruct->get_name() << "::";
  }
...
void t_dpic_generator::generate_exception_what_method_decl(std::ostream& out,
                                                          t_struct* tstruct,
                                                          bool external) {
  out << "const char* ";
  if (external) {
...
namespace dpic_struct_ostream_operator_generator {
 ...
 * Generates operator<<
 */
void t_dpic_generator::generate_struct_print_method(std::ostream& out, t_struct* tstruct) {
  out << indent();
  ...
  dpic_struct_ostream_operator_generator::generate_fields(out, tstruct->get_members(), indent());
  out << indent() << "out << \")\";" << '\n';
 ...
 */
void t_dpic_generator::generate_exception_what_method(std::ostream& out, t_struct* tstruct) {
  out << indent();
 ...
 */
void t_dpic_generator::generate_service(t_service* tservice) {
 ...
 */
void t_dpic_generator::generate_service_helpers(t_service* tservice) {
 ...
 * @param tservice The service to generate a header definition for
 */
void t_dpic_generator::generate_service_interface(t_service* tservice, string style) {
 ...
 */
void t_dpic_generator::generate_service_interface_factory(t_service* tservice, string style) {
  string service_if_name = service_name_ + style + "If";
 ...
 * @param tservice The service to generate a header definition for
 */
void t_dpic_generator::generate_service_null(t_service* tservice, string style) {
  string extends = "";
  if (tservice->get_extends() != nullptr) {
...
void t_dpic_generator::generate_function_call(ostream& out,
                                             t_function* tfunction,
                                             string target,
                                             string iface,
                                             string arg_prefix) {
...
void t_dpic_generator::generate_service_async_skeleton(t_service* tservice) {
  string svcname = tservice->get_name();
 ...
 */
void t_dpic_generator::generate_service_multiface(t_service* tservice) {
 ...
 */
void t_dpic_generator::generate_service_client(t_service* tservice, string style) {
  string ifstyle;
  if (style == "Cob") {
    ifstyle = "CobCl";
  }
...
class DpicProcessorGenerator {
public:
  DpicProcessorGenerator(t_dpic_generator* generator, t_service* service, const string& style);
...
DpicProcessorGenerator::DpicProcessorGenerator(t_dpic_generator* generator,
                                       t_service* service,
                                       const string& style)
...
void DpicProcessorGenerator::generate_class_definition() {
  // Generate the dispatch methods
...
void DpicProcessorGenerator::generate_dispatch_call(bool template_protocol) {
...
void DpicProcessorGenerator::generate_process_functions() {
  vector<t_function*> functions = service_->get_functions();
  vector<t_function*>::iterator f_iter;
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    if (generator_->gen_templates_) {
...
void DpicProcessorGenerator::generate_factory() {
 ...
 */
void t_dpic_generator::generate_service_processor(t_service* tservice, string style) {
  DpicProcessorGenerator generator(this, tservice, style);
  generator.run();
}
 ...
 */
void t_dpic_generator::generate_function_helpers(t_service* tservice, t_function* tfunction) {
 ...
 */
void t_dpic_generator::generate_process_function(t_service* tservice,
                                                t_function* tfunction,
                                                string style,
                                                bool specialized) {
 ...
 * @param tservice The service to generate a server for.
 */
void t_dpic_generator::generate_service_skeleton(t_service* tservice) {
  string svcname = tservice->get_name();
 ...
 */
void t_dpic_generator::generate_deserialize_field(ostream& out,
                                                 t_field* tfield,
                                                 string prefix,
                                                 string suffix) {
 ...
 */
void t_dpic_generator::generate_deserialize_struct(ostream& out,
                                                  t_struct* tstruct,
                                                  string prefix,
                                                  bool pointer) {
  if (pointer) {
...
void t_dpic_generator::generate_deserialize_container(ostream& out, t_type* ttype, string prefix) {
  scope_up(out);
 ...
 */
void t_dpic_generator::generate_deserialize_map_element(ostream& out, t_map* tmap, string prefix) {
  string key = tmp("_key");
  string val = tmp("_val");
...
void t_dpic_generator::generate_deserialize_set_element(ostream& out, t_set* tset, string prefix) {
...
void t_dpic_generator::generate_deserialize_list_element(ostream& out,
                                                        t_list* tlist,
                                                        string prefix,
                                                        bool use_push,
                                                        string index) {
 ...
 */
void t_dpic_generator::generate_serialize_field(ostream& out,
                                               t_field* tfield,
                                               string prefix,
                                               string suffix) {
  t_type* type = get_true_type(tfield->get_type());
 ...
 */
void t_dpic_generator::generate_serialize_struct(ostream& out,
                                                t_struct* tstruct,
                                                string prefix,
                                                bool pointer) {
  if (pointer) {
...
void t_dpic_generator::generate_serialize_container(ostream& out, t_type* ttype, string prefix) {
  scope_up(out);
 ...
 *
 */
void t_dpic_generator::generate_serialize_map_element(ostream& out, t_map* tmap, string iter) {
 ...
 */
void t_dpic_generator::generate_serialize_set_element(ostream& out, t_set* tset, string iter) {
 ...
 */
void t_dpic_generator::generate_serialize_list_element(ostream& out, t_list* tlist, string iter) {
 ...
 * @return Namespaces
 */
string t_dpic_generator::namespace_prefix(string ns) {
 ...
 * @return Namespaces
 */
string t_dpic_generator::namespace_open(string ns) {
  if (ns.size() == 0) {
    return "";
  }
  string result = "";
 ...
 * @return Namespaces
 */
string t_dpic_generator::namespace_close(string ns) {
  if (ns.size() == 0) {
    return "";
  }
  string result = "}";
 ...
 */
string t_dpic_generator::type_name(t_type* ttype, bool in_typedef, bool arg) {
 ...
 */
string t_dpic_generator::base_type_name(t_base_type::t_base tbase) {
  switch (tbase) {
  case t_base_type::TYPE_VOID:
    return "void";
  case t_base_type::TYPE_STRING:
 ...
 */
string t_dpic_generator::declare_field(t_field* tfield,
                                      bool init,
                                      bool pointer,
                                      bool constant,
                                      bool reference) {
 ...
 */
string t_dpic_generator::function_signature(t_function* tfunction,
                                           string style,
                                           string prefix,
                                           bool name_params) {
 ...
 */
string t_dpic_generator::argument_list(t_struct* tstruct, bool name_params, bool start_comma) {
  string result = "";
 ...
 */
string t_dpic_generator::type_to_enum(t_type* type) {
  type = get_true_type(type);
...
bool t_dpic_generator::is_struct_storage_not_throwing(t_struct* tstruct) const {
...
string t_dpic_generator::get_include_prefix(const t_program& program) const {
...
string t_dpic_generator::get_legal_program_name(std::string program_name)
{
  std::size_t found = 0;
...
std::string t_dpic_generator::display_name() const {
  return "DPI-C";
}
