 ...
 *
 */
class t_sv_generator : public t_oop_generator {
public:
  t_sv_generator(t_program* program,
    ...
    for( iter = parsed_options.begin(); iter != parsed_options.end(); ++iter) {
      if( iter->first.compare("pure_enums") == 0) {
        ...
        throw "unknown option sv:" + iter->first;
      }
    }

    out_dir_base_ = "gen-sv";
  }
  ...
  void generate_dpic_declaration(ostream& out,
                                  t_struct* tstruct);
  ...
  std::string sv_type_formatter(t_field* tfield);
   ...
   * the annotation "sv.customostream" to prevent thrift from emitting an
  ...
  bool has_custom_ostream(t_type* ttype) const {
    return (gen_no_ostream_operators_) ||
           (ttype->annotations_.find("sv.customostream") != ttype->annotations_.end());
  }
 ...
 * streams.
 */
void t_sv_generator::init_generator() {
  ...
  // Make output file
  string f_types_name = get_out_dir() + program_name_ + "_types.svh";
  f_types_.open(f_types_name);

  string f_types_impl_name = get_out_dir() + program_name_ + "_types.sv";
  ...
  // Start ifndef
  f_types_ << "`ifndef " << program_name_ << "_TYPES_SVH" << '\n' << "`define " << program_name_
           << "_TYPES_SVH" << '\n' << '\n';
  ...
  // Open namespace
  ns_open_ = namespace_open(program_->get_namespace("sv"));
  ns_close_ = namespace_close(program_->get_namespace("sv"));
 ...
 */
void t_sv_generator::close_generator() {
  // Close namespace
  f_types_ << ns_close_ << '\n' << '\n';
 ...
 */
void t_sv_generator::generate_typedef(t_typedef* ttypedef) {
  generate_java_doc(f_types_, ttypedef);
  f_types_ << indent() << "typedef " << type_name(ttypedef->get_type(), true) << " "
           << ttypedef->get_symbolic() << ";" << '\n' << '\n';
}

void t_sv_generator::generate_enum_constant_list(std::ostream& f,
                                                  const vector<t_enum_value*>& constants,
                                                  const char* prefix,
                                                  const char* suffix,
                                                  bool include_values) {
 ...
 */
void t_sv_generator::generate_enum(t_enum* tenum) {
...
void t_sv_generator::generate_enum_ostream_operator_decl(std::ostream& out, t_enum* tenum) {
...
void t_sv_generator::generate_enum_ostream_operator(std::ostream& out, t_enum* tenum) {
...
void t_sv_generator::generate_enum_to_string_helper_function_decl(std::ostream& out, t_enum* tenum) {
...
void t_sv_generator::generate_enum_to_string_helper_function(std::ostream& out, t_enum* tenum) {
  if (!has_custom_ostream(tenum)) {
    out << "std::string to_string(const ";
    if (gen_pure_enums_) {
      out << tenum->get_name();
 ...
 */
void t_sv_generator::generate_consts(std::vector<t_const*> consts) {
  string f_consts_name = get_out_dir() + program_name_ + "_constants.h";
  ofstream_with_content_based_conditional_update f_consts;
  if (consts.size() > 0) {
    f_consts.open(f_consts_name);

    string f_consts_impl_name = get_out_dir() + program_name_ + "_constants.sv";
 ...
 */
void t_sv_generator::print_const_value(ostream& out,
                                        string name,
                                        t_type* type,
                                        t_const_value* value) {
 ...
 *
 */
string t_sv_generator::render_const_value(ostream& out,
                                           string name,
                                           t_type* type,
                                           t_const_value* value) {
  (void)name;
...
void t_sv_generator::generate_forward_declaration(t_struct* tstruct) {
 ...
 */
void t_sv_generator::generate_cpp_struct(t_struct* tstruct, bool is_exception) {

  generate_dpic_declaration(f_types_, tstruct);
...
void t_sv_generator::generate_equality_operator(std::ostream& out, t_struct* tstruct) {
  // Get members
...
bool t_sv_generator::has_field_with_default_value(t_struct* tstruct)
{
  vector<t_field*>::const_iterator m_iter;
  const vector<t_field*>& members = tstruct->get_members();
...
void t_sv_generator::generate_default_constructor(ostream& out,
                                                   t_struct* tstruct,
                                                   bool is_exception) {
  // Get members
  vector<t_field*>::const_iterator m_iter;
...
void t_sv_generator::generate_copy_constructor(ostream& out,
                                                t_struct* tstruct,
                                                bool is_exception) {
  generate_constructor_helper(out, tstruct, is_exception, /*is_move=*/false);
}

void t_sv_generator::generate_move_constructor(ostream& out,
                                                t_struct* tstruct,
                                                bool is_exception) {
...
void t_sv_generator::generate_constructor_helper(ostream& out,
                                                  t_struct* tstruct,
                                                  bool is_exception,
                                                  bool is_move) {
...
void t_sv_generator::generate_assignment_operator(ostream& out, t_struct* tstruct) {
  generate_assignment_helper(out, tstruct, /*is_move=*/false);
}

void t_sv_generator::generate_move_assignment_operator(ostream& out, t_struct* tstruct) {
  generate_assignment_helper(out, tstruct, /*is_move=*/true);
}

void t_sv_generator::generate_assignment_helper(ostream& out, t_struct* tstruct, bool is_move) {
  std::string tmp_name = tmp("other");
...
std::string t_sv_generator::sv_type_formatter(t_field* tfield) {
...
void t_sv_generator::generate_dpic_declaration(ostream& out,
                                  t_struct* tstruct) {
  ...
  out << "import \"DPI-C\" context function chandle deserialize_" << tstruct->get_name() << "_from_file(input string filename);\n";
  out << "import \"DPI-C\" context function void display_" << tstruct->get_name() << "(input chandle ptr);\n";
  ...
  for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
    ...
    if (type->is_base_type()) {
      out << "import \"DPI-C\" context function void read_" << tstruct->get_name() << "_" << name 
          << "(input chandle ptr, output " << type_name((*m_iter)->get_type()) << " " << name << ");\n";
    }
    else {
      //out << "import \"DPI-C\" context function chandle read_" << tstruct->get_name() << "_" << name 
      //    << "(input chandle ptr);\n";
      out << "import \"DPI-C\" context function void read_" << tstruct->get_name() << "_" << name 
          << "(input chandle ptr, output chandle ptr" << name << ");\n";
    }
  }
}
 ...
 */
void t_sv_generator::generate_struct_declaration(ostream& out,
                                                  t_struct* tstruct,
                                                  bool is_exception,
                                                  bool pointers,
                                                  bool read,
  ...
  for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
    ...
    if (type->is_base_type()) {
      //indent(out) << "read_" << tstruct->get_name() << "_" << name << "(" << "cptr, " << name << ");\n";
      //indent(out) << "$display(\"\%\s\","<< name << ");\n";
    }
    else {
      indent(out) << name <<" = new;\n";
      //indent(out) << "read_" << tstruct->get_name() << "_" << name << "(" << "cptr, " << name << ".cptr);\n";
      //indent(out) << name <<".cptr = read_" << tstruct->get_name() << "_" << name << "(" << "cptr);\n";
      //indent(out) << name <<".read_data_from_cobj();\n";
    }
  }

  indent_down();
  indent(out) << "endfunction\n\n\n";

  //============================================================
  // function void deserialize_from_file(input string filename);
  indent(out) << "function void deserialize_from_file(input string filename);\n";
  indent_up();
  indent(out) << "cptr = deserialize_" << tstruct->get_name() <<"_from_file(filename);\n";
  indent(out) << "read_data_from_cobj();\n";
  indent_down();
  indent(out) << "endfunction\n\n\n";


  //============================================================
  // function void read_data_from_cobj();
  indent(out) << "function void read_data_from_cobj();\n";
  indent_up();
  for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
    string name = (*m_iter) -> get_name();
    t_type* type = get_true_type((*m_iter)->get_type());
    ...
    if (type->is_base_type()) {
      indent(out) << "read_" << tstruct->get_name() << "_" << name << "(" << "cptr, " << name << ");\n";
      indent(out) << "$display(\"" << sv_type_formatter(*m_iter)  <<"\","<< name << ");\n";
    }
    else {
      indent(out) << "read_" << tstruct->get_name() << "_" << name << "(" << "cptr, " << name << ".cptr);\n";
      //indent(out) << name <<".cptr = read_" << tstruct->get_name() << "_" << name << "(" << "cptr);\n";
  ...
  indent(out) << "function void display(input int indent = 0);\n";
  indent_up();
  //indent(out) << "$display(\"Class);\n"
  for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
    string name = (*m_iter) -> get_name();
    t_type* type = get_true_type((*m_iter)->get_type());

    if (type->is_base_type()) {
      indent(out) << "$display(\"%s"<< name <<": " << sv_type_formatter(*m_iter) << "\",{indent{\" \"}} , " << name << ");\n";
    }
    else {
      indent(out) << "$display(\"%s" << name << ":\", {indent{\" \"}});\n";
      indent(out) << name << ".display(indent+4);\n";
    }
  }
  indent_down();
  indent(out) << "endfunction\n\n\n";
  ...
  //===========================================================
  // function void display_by_dpic();
  indent(out) << "function void display_by_dpic();\n";
  indent_up();
  indent(out) << "display_" << tstruct->get_name() << "(cptr);\n";
  indent_down();
  indent(out) << "endfunction\n\n\n";
...
// string t_sv_generator::declare_field(t_field* tfield,
//                                       bool init,
//                                       bool pointer,
//                                       bool constant,
//                                       bool reference) {
...
void t_sv_generator::generate_struct_definition(ostream& out,
  ...
  if (!pointers)
  {
    // 'force_cpp_out' always goes into the .sv file, and never into a .tcc
 ...
 * @param tstruct The struct
 */
void t_sv_generator::generate_struct_reader(ostream& out, t_struct* tstruct, bool pointers) {
  if (gen_templates_) {
    out << indent() << "template <class Protocol_>" << '\n' << indent() << "uint32_t "
 ...
 * @param tstruct The struct
 */
void t_sv_generator::generate_struct_writer(ostream& out, t_struct* tstruct, bool pointers) {
 ...
 */
void t_sv_generator::generate_struct_result_writer(ostream& out,
                                                    t_struct* tstruct,
                                                    bool pointers) {
  string name = tstruct->get_name();
  const vector<t_field*>& fields = tstruct->get_sorted_members();
 ...
 * @param tstruct The struct
 */
void t_sv_generator::generate_struct_swap(ostream& out, t_struct* tstruct) {
  if (tstruct->get_name() == "a" || tstruct->get_name() == "b") {
    out << indent() << "void swap(" << tstruct->get_name() << " &a1, " << tstruct->get_name()
        << " &a2) {" << '\n';
  } else {
...
void t_sv_generator::generate_struct_ostream_operator_decl(std::ostream& out, t_struct* tstruct) {
  out << "std::ostream& operator<<(std::ostream& out, const "
      << tstruct->get_name()
      << "& obj);" << '\n';
  out << '\n';
...
void t_sv_generator::generate_struct_ostream_operator(std::ostream& out, t_struct* tstruct) {
  if (!has_custom_ostream(tstruct)) {
...
void t_sv_generator::generate_struct_print_method_decl(std::ostream& out, t_struct* tstruct) {
  out << "void ";
  if (tstruct) {
    out << tstruct->get_name() << "::";
  }
...
void t_sv_generator::generate_exception_what_method_decl(std::ostream& out,
                                                          t_struct* tstruct,
                                                          bool external) {
  out << "const char* ";
  if (external) {
 ...
 * Generates operator<<
 */
void t_sv_generator::generate_struct_print_method(std::ostream& out, t_struct* tstruct) {
  out << indent();
 ...
 */
void t_sv_generator::generate_exception_what_method(std::ostream& out, t_struct* tstruct) {
  out << indent();
 ...
 */
void t_sv_generator::generate_service(t_service* tservice) {
  string svcname = tservice->get_name();

  // Make output files
  string f_header_name = get_out_dir() + svcname + ".svh";
  ...
  string f_service_name = get_out_dir() + svcname + ".sv";
 ...
 */
void t_sv_generator::generate_service_helpers(t_service* tservice) {
 ...
 * @param tservice The service to generate a header definition for
 */
void t_sv_generator::generate_service_interface(t_service* tservice, string style) {
 ...
 */
void t_sv_generator::generate_service_interface_factory(t_service* tservice, string style) {
  string service_if_name = service_name_ + style + "If";
 ...
 * @param tservice The service to generate a header definition for
 */
void t_sv_generator::generate_service_null(t_service* tservice, string style) {
  string extends = "";
  if (tservice->get_extends() != nullptr) {
...
void t_sv_generator::generate_function_call(ostream& out,
                                             t_function* tfunction,
                                             string target,
                                             string iface,
                                             string arg_prefix) {
...
void t_sv_generator::generate_service_async_skeleton(t_service* tservice) {
  string svcname = tservice->get_name();

  // Service implementation file includes
  string f_skeleton_name = get_out_dir() + svcname + "_async_server.skeleton.sv";

  string ns = namespace_prefix(tservice->get_program()->get_namespace("sv"));
 ...
 */
void t_sv_generator::generate_service_multiface(t_service* tservice) {
 ...
 */
void t_sv_generator::generate_service_client(t_service* tservice, string style) {
  string ifstyle;
  if (style == "Cob") {
    ifstyle = "CobCl";
  }
...
public:
  ProcessorGenerator(t_sv_generator* generator, t_service* service, const string& style);
...
ProcessorGenerator::ProcessorGenerator(t_sv_generator* generator,
                                       t_service* service,
                                       const string& style)
 ...
 */
/*
void t_sv_generator::generate_service_processor(t_service* tservice, string style) {
 ...
 */
void t_sv_generator::generate_function_helpers(t_service* tservice, t_function* tfunction) {
 ...
 */
void t_sv_generator::generate_process_function(t_service* tservice,
                                                t_function* tfunction,
                                                string style,
                                                bool specialized) {
 ...
 * @param tservice The service to generate a server for.
 */
void t_sv_generator::generate_service_skeleton(t_service* tservice) {
  string svcname = tservice->get_name();

  // Service implementation file includes
  string f_skeleton_name = get_out_dir() + svcname + "_server.skeleton.sv";
 ...
 */
void t_sv_generator::generate_deserialize_field(ostream& out,
                                                 t_field* tfield,
                                                 string prefix,
                                                 string suffix) {
  t_type* type = get_true_type(tfield->get_type());
 ...
 */
void t_sv_generator::generate_deserialize_struct(ostream& out,
                                                  t_struct* tstruct,
                                                  string prefix,
                                                  bool pointer) {
  if (pointer) {
...
void t_sv_generator::generate_deserialize_container(ostream& out, t_type* ttype, string prefix) {
  scope_up(out);
 ...
 */
void t_sv_generator::generate_deserialize_map_element(ostream& out, t_map* tmap, string prefix) {
  string key = tmp("_key");
  string val = tmp("_val");
...
void t_sv_generator::generate_deserialize_set_element(ostream& out, t_set* tset, string prefix) {
...
void t_sv_generator::generate_deserialize_list_element(ostream& out,
                                                        t_list* tlist,
                                                        string prefix,
                                                        bool use_push,
                                                        string index) {
 ...
 */
void t_sv_generator::generate_serialize_field(ostream& out,
                                               t_field* tfield,
                                               string prefix,
                                               string suffix) {
  t_type* type = get_true_type(tfield->get_type());
 ...
 */
void t_sv_generator::generate_serialize_struct(ostream& out,
                                                t_struct* tstruct,
                                                string prefix,
                                                bool pointer) {
  if (pointer) {
...
void t_sv_generator::generate_serialize_container(ostream& out, t_type* ttype, string prefix) {
  scope_up(out);
 ...
 *
 */
void t_sv_generator::generate_serialize_map_element(ostream& out, t_map* tmap, string iter) {
 ...
 */
void t_sv_generator::generate_serialize_set_element(ostream& out, t_set* tset, string iter) {
 ...
 */
void t_sv_generator::generate_serialize_list_element(ostream& out, t_list* tlist, string iter) {
 ...
 * @return Namespaces
 */
string t_sv_generator::namespace_prefix(string ns) {
 ...
 * @return Namespaces
 */
string t_sv_generator::namespace_open(string ns) {
  if (ns.size() == 0) {
    return "";
  }
  string result = "";
 ...
 * @return Namespaces
 */
string t_sv_generator::namespace_close(string ns) {
  if (ns.size() == 0) {
    return "";
  }
  string result = "}";
 ...
 */
string t_sv_generator::type_name(t_type* ttype, bool in_typedef, bool arg) {
  if (ttype->is_base_type()) {
    string bname = base_type_name(((t_base_type*)ttype)->get_base());
    std::map<string, std::vector<string>>::iterator it = ttype->annotations_.find("sv.type");
  ...
  if (program != nullptr && program != program_) {
    pname = class_prefix + namespace_prefix(program->get_namespace("sv")) + ttype->get_name();
  } else {
 ...
 */
string t_sv_generator::base_type_name(t_base_type::t_base tbase) {
  switch (tbase) {
  case t_base_type::TYPE_VOID:
    return "void";
  case t_base_type::TYPE_STRING:
 ...
 */
string t_sv_generator::function_signature(t_function* tfunction,
                                           string style,
                                           string prefix,
                                           bool name_params) {
 ...
 */
string t_sv_generator::argument_list(t_struct* tstruct, bool name_params, bool start_comma) {
  string result = "";
 ...
 */
string t_sv_generator::type_to_enum(t_type* type) {
  type = get_true_type(type);
...
bool t_sv_generator::is_struct_storage_not_throwing(t_struct* tstruct) const {
...
string t_sv_generator::get_include_prefix(const t_program& program) const {
...
string t_sv_generator::get_legal_program_name(std::string program_name)
{
  std::size_t found = 0;
...
std::string t_sv_generator::display_name() const {
